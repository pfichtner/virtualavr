...
class ArdulinkAvrTest {
	...
	VirtualAvrContainer<?> virtualavr = new VirtualAvrContainer<>()
			.withSketchFile(new File("/tmp/ArdulinkProtocol.ino.hex"));

	@Test
	void canTurnOnAndOffDigital() throws Exception {
		int pinNumber = 12;
		String pin = format("D%d", pinNumber);
		VirtualAvrConnection avr = withDebugEnabled();
	...
	private VirtualAvrConnection withDebugEnabled() {
	...
	private SerialConnection steadyConnection() throws IOException {
		SerialConnection serialConnection = virtualavr.serialConnection();
		awaitSteadyMessage();
	...
	private void serialAwait(Consumer<AbstractStringAssert<?>> consumer) throws IOException {
		SerialConnection serial = virtualavr.serialConnection();
	...
	private void awaitState(Consumer<ListAssert<PinState>> consumer) {
		await().untilAsserted(() -> consumer.accept(assertThat(virtualavr.avr().pinStates())));
	}
